...
class MinCostTimeJob < ApplicationJob
  ...
  def perform(*args)
    Cloudlet.all.each do |cloudlet|
      ...
      [
        :greedy_algorithm, :dynamic_programming, :backtracking, 
  ...
  # [Keep all your algorithm methods here exactly as you had them]
  # greedy_algorithm, dynamic_programming, etc.
  # ...
end
  ...
  def pso(w1, w2, sim_results)
...

    {
      ...
      instance_type_id: best_sim.vm_id,
      datacenter_id: best_sim.datacenter_id,
      algo_eff_time: 0 # PSO is instantaneous, so no benchmark time
    }
  end
  ...
  def smo(w1, w2, sim_results)
...

    {
      ...
      instance_type_id: best_sim.vm_id,
      datacenter_id: best_sim.datacenter_id,
      algo_eff_time: 0 # SMO is instantaneous, so no benchmark time
    }
  end
  ...
  def pso_smo_hybrid(w1, w2, sim_results)
...

    {
      ...
      instance_type_id: best_sim.vm_id,
      datacenter_id: best_sim.datacenter_id,
      algo_eff_time: 0 # Hybrid PSO-SMO is instantaneous, so no benchmark time
    }
  end
end